            ...
            {channels[i]?.purpose !== 'SPARE' && (
              <>
                ...
                <Field label="Load Description">
                  <Input value={channels[i]?.load_description || ''} onChange={e => setChannel(i, 'load_description', e.target.value)} placeholder={channels[i]?.purpose === 'SUB_CIRCUIT' ? 'e.g. Rooftop HVAC Unit 1 (assign to asset later)' : 'e.g. Grid Connect - Phase A'} />
                </Field>
              </>
            )}
          </div>
